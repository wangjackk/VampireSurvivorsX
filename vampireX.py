...
def generate_hero_txt_from_original_txt(bundle_path, save_dic_path=''):
    ...
    for hero in heroes_dic:
        ...
        hero_data.append((name, pos))
    max_hero_num = len(hero_data)
    pos_data_ = []
    for number, hero in enumerate(hero_data):
        name, start = hero
        if number < max_hero_num - 1:
            end = hero_data[number+1][-1]
        else:
            end = len(bundle)
        pos_data_.append((name, (start, end)))

    heroes_names = []
    for data in pos_data_:
        name, pos = data
        s, e = pos
        text = bundle[s:e]
        ...
        n = 0
        name_ = name
        while name_ in heroes_names:
            n += 1
            name_ = name + str(n)

        heroes_names.append(name_)
        name = os.path.join(save_dic_path, name_.strip("'") + '.txt')
...
def checkHeroesUpdated():
    newest_heroes_list = HeroesInfo.old_heroes_dict.keys()
    if newest_heroes_list != heroes:
        return True
    return False
